...
| Vertailu lohkon sisällä | `.vertailu` · `table.data.stack.vertailutaulu` · `.pf .pf-im` | hubi (perheet) | **ACF** · ei itsenäinen lohko vaan Perhekorttien jatke: taulukon kaksi riviä lasketaan mittataulukosta, joten `core/table` antaisi kirjoittaa ne käsin | Lisätty 23.9.2026. Lohkon toinen alaosa: erotinviiva, H3 ja ingressi, sitten taulukko ja `.fine`-huomautus. Sama mitta ja sama erotin kuin `.find`:llä samassa lohkossa, jotta yhden lohkon kaksi alaosaa lukevat samalla rytmillä. **Rivi on tuote ja sarake on ominaisuus**, ja rivin ensimmäinen solu on `.pf` = nimi ja sen alla tuotekuva. Nimi on kuvan yläpuolella, koska vierekkäin ladottuna rivin tunniste jäi sata pikseliä alemmas kuin rivin arvot. Kuvat ovat jaetussa mittakaavassa mutta **omilla kankaillaan** eikä `tuotekuva.rivi`:n yhteisellä: yhteinen kangas on oikein vierekkäisille korteille, mutta allekkaisilla taulukkoriveillä kuvan korkeus itse on se vertailu (AP 88 px, KP 150 px). Kolme ansaa, kaikki nähty tätä tehdessä: `.vertailutaulu` yksin ei kumoa `.data.stack`:in 900 px:ää (kirjoita `.data.stack.vertailutaulu`); kuvan svg ei saa `max-width:100%`:a, koska taulukon sarake mitoitetaan min-content-leveydestä ja prosenttiraja tekee siitä nollan — oire oli, että kuva valui nimen päälle; ingressin sääntö on `.vertailu h3+p` eikä `.vertailu>p`, joka voittaisi myös `.fine`:n |
