...
| Kuvan selite listana | `ol.kuvaselite > li > b + span` | autokatos | **core** · `core/image` + `core/list` samassa `core/group`:ssa, lohkotyyli *kuvaselite* | Uusi 23.9.2026. **Kuvaan ei ladota selitetekstiä.** Mitattu: kangas skaalautuu 390 px:n ruudulla 342 pikseliin, jolloin 15 px:n selite on 3,8 px — mobiili on 80,9 % istunnoista, ja työpöydälläkin selite jää 12,9 pikseliin. Isompi pistekoko ei ratkaise sitä, koska sama kangas on työpöydällä 1 152 px ja puhelimessa 342 px. Kuvaan jää numeroitu merkki, teksti tulee listana kuvan alle: se ei skaalaudu, se on käännettävissä ja ruudunlukija lukee sen. Numeron pohja on `--accent-dark`, sama sävy kuin kuvan merkissä — side on väri ja numero, joten kumpaakaan ei muuteta yksin |
