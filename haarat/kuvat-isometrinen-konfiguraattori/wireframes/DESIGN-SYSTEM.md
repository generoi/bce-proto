...
| Tekstikärki hakemistolla | `.lead` + `.idx` | — (tarvikesivulta pois 23.9.2026) | **ACF** · `bce/sivuhakemisto` generoi linkit H2-otsikoista; kärki itse on lohkomalli | `.topline` (A, kuollut 16.9.) |
| Tekstikärki ilman hakemistoa | `.lead.solo` | Tarvikkeet | **Lohkomalli** · sama kärki yhdellä palstalla | Modifier on pakollinen: `.lead` on kaksipalstainen (1fr + 380px), ja yhdellä lapsella teksti puristuisi vasempaan palstaan oikean jäädessä tyhjäksi |
| Ankkuririvi | `.anchors a (.ic b .ar .n)` | Tarvikkeet | **ACF** · `bce/sivuhakemisto`, kuvallinen variantti — ei viidestoista custom-lohko | Yksi pikalinkki per tuoteryhmä kärjen alle: havainnekuva 56 px, nimi, nimikkeiden määrä, ↓. **Ei kortti:** ei valkoista levyä, ei kehystä, ei `.ph`-kuvapaikkaa, ja yläviiva sitoo neljä linkkiä nauhaksi. Nuoli on ↓ eikä →, koska kohde on samalla sivulla. Korvasi 23.9.2026 sekä `.idx`:n kahdeksan tuotelinkkiä että `.pick-how`-suodattimen. Kuva piirretään 140 px:iin ja mitoitetaan CSS:llä — generaattorin kiinteä 18 px:n reunus tekisi pienenä piirretystä puolet luvatusta |
...
| Aito kuva omalla rivillään | `figure.figwide > img` (+ `figcaption`) | autokatos | **core** · `core/image`, tasaus wide tai full | Lisätty 17.9.2026. Leveä piirros ei mahdu mediaparin palstaan (552 px): selite renderöityi 5 px:n kokoisena. Kuvapaikka on yhä `.ph`; tämä on aidolle kuvalle |
| Callout-selite piirroksen päällä | `.co-lava` > `.co-kuva` (`img` + `svg.co-viivat` + `b.co-nro`) + `ol.co-teksti` | autokatos | **core** · `core/image` + `core/list` samassa `core/group`:ssa, lohkotyyli *callout*; merkkien paikat ACF-kenttinä | Uusi 23.9.2026. Callout on merkki kuvassa, johtoviiva ja selite viivan päässä — ainoa ero tavalliseen on se, että merkki, viiva ja teksti ovat kuvan **päällä** eivätkä kuvassa. **Kuvaan ei ladota selitetekstiä.** Mitattu: kangas skaalautuu 390 px:n ruudulla 342 pikseliin, jolloin 15 px:n selite on 3,8 px — mobiili on 80,9 % istunnoista, ja työpöydälläkin selite jää 12,9 pikseliin. Isompi pistekoko ei ratkaise sitä, koska sama kangas on työpöydällä 1 152 px ja puhelimessa 342 px. Merkin, viivan ja tekstin koko on CSS:ssä eikä kankaalla, joten ne pysyvät samoina joka leveydellä: teksti on 17 px, se on käännettävissä ja ruudunlukija lukee sen. Merkkien paikat tulevat generaattorilta (`kuvat/asennus-callout.js`, prosentteina kuvasta), joten ne eivät voi ajautua erilleen piirroksesta; selitteet sivu jakaa tasavälein, koska selitteen korkeus on pikseleitä eikä skaalaudu. Numeron pohja on `--accent-dark`. Alle 820 px callout purkautuu numeroiduksi listaksi: merkit jäävät kuvaan, viivat ja palsta katoavat, koska 342 px:n kuvaan ei mahdu selitepalstaa. **Ansa:** `.figwide svg{height:auto}` (0,1,1) voittaa paljaan `.co-viivat`-säännön, joten peitto tarvitsee kaksi luokkaa |
| Latauskortti | `.dl` | TP, B | **ACF** · `bce/lataus`. Laskeutumissivu ja mitattu tapahtuma, joten paljas linkki ei kelpaa | `.dl-1` (kuollut) |
| Suodatinkortti | `.pick-how a (.ph-1610 .tx b .d)` + `.fstate .lnk` | Tarvikkeet B | **ACF** · `bce/tarvikesuodatin`. Aito custom: korostaa, piilottaa, kuittaa ruudunlukijalle. JS-riippuvuus | **Piilottaa sopimattomat** (`.tcard[hidden]`, tyhjä ryhmä `.tgroup[hidden]`), **ei vieritä** — vaimennettu 400 px:n rivi on este, ei tieto. Kaavio kortissa kertoo liitostyypin; 16:10, koska 3:1 on liian matala oikealle kuvalle. Mobiilissa kortti kääntyy vaakaan (kuva 120 px vasemmalla), muuten viisi korttia veisi kaksi ruudullista. **Kuittausrivillä näkyy vain purkupainike ja vain kun valinta on päällä**; määrä jää `.sr[role=status]`-alueeseen ruudunlukijalle. `.fstate .msg`, `.fstate .more`, `.hitlbl`, `.hit` ja `.dim` kuolleet |
...
| Pinoutuva taulukko | `table.data.stack` (`th[scope=row]`, `td[data-lbl]`) | Perustustavat, hubi (perhevertailu) | **core** · `core/table`, lohkotyyli *stack*. Sisältö on sivukohtaista copya | Lisätty 18.9.2026. Alle 560 px otsikkorivi piiloon ja jokainen rivi omaksi lohkokseen; sarakeotsikko tulee `data-lbl`-attribuutista, joka täytetään samasta `C`-objektista kuin `thead`. Käytä kun taulukko on koko sivun tiivistelmä. `.dims` tekee päinvastoin (vaakavieritys), koska ammattilainen etsii yhtä lukua. **Hubin perhevertailu 23.9.2026 käyttää samaa akselia** — rivi on tuote — mutta kahdeksalla sarakkeella ja rivikuvalla, ks. `.vertailutaulu` seuraavalla rivillä. Ensimmäinen versio oli toisin päin (rivi = ominaisuus); se luki hyvin muttei vastannut yläpuolella olevien korttien järjestystä, ja vaihdettiin samana päivänä |
| Vertailu lohkon sisällä | `.vertailu` · `table.data.stack.vertailutaulu` · `.pf .pf-im` | hubi (perheet) | **ACF** · ei itsenäinen lohko vaan Perhekorttien jatke: taulukon kaksi riviä lasketaan mittataulukosta, joten `core/table` antaisi kirjoittaa ne käsin | Lisätty 23.9.2026. Lohkon toinen alaosa: erotinviiva, H3 ja ingressi, sitten taulukko ja `.fine`-huomautus. Sama mitta ja sama erotin kuin `.find`:llä samassa lohkossa, jotta yhden lohkon kaksi alaosaa lukevat samalla rytmillä. **Rivi on tuote ja sarake on ominaisuus**, ja rivin ensimmäinen solu on `.pf` = nimi ja sen alla tuotekuva. Nimi on kuvan yläpuolella, koska vierekkäin ladottuna rivin tunniste jäi sata pikseliä alemmas kuin rivin arvot. Kuvat ovat jaetussa mittakaavassa ja **yhteisellä kankaalla** (`tuotekuva.rivi`, 85 × 96 px), joten neljä riviä ovat yhtä korkeita. Omaa kangasta kullekin kokeiltiin 23.9.2026 ja hylättiin samana päivänä: kuvan korkeus oli silloin itse vertailu eikä lyhyen tuotteen yläpuolelle jäänyt tyhjää, mutta eri korkuiset rivit maksoivat enemmän kuin tyhjä voitti — silmä lukee taulukkoa riviväliä pitkin. **Arvot eivät rivity leveällä** (`.vertailutaulu td{white-space:nowrap}`, pois alle 561 px): kun jokainen arvo on yksi rivi, taulukolla on yksi rytmi eikä kahdeksaa — tämä on se yksittäinen sääntö, joka teki kahdeksan sarakkeen taulukosta luettavan. Automaattinen sarakejako antaa rivittymättömille sarakkeille tasan niiden sisällön leveyden ja loput ainoalle rivittyvälle (`td.sanat`, taulukon ainoa lause). **Sisältö on lyhennettävä sen mukaan:** jos arvo ei mahdu, se on liian pitkä soluun eikä sarake liian kapea, ja oikea paikka on `.fine`-alaviite. Numerot ovat `tabular-nums`. Kolme ansaa, kaikki nähty tätä tehdessä: `.vertailutaulu` yksin ei kumoa `.data.stack`:in 900 px:ää (kirjoita `.data.stack.vertailutaulu`); kuvan svg ei saa `max-width:100%`:a, koska taulukon sarake mitoitetaan min-content-leveydestä ja prosenttiraja tekee siitä nollan — oire oli, että kuva valui nimen päälle; ingressin sääntö on `.vertailu h3+p` eikä `.vertailu>p`, joka voittaisi myös `.fine`:n |
...
| Konfiguraattorin nimi | **tehty 23.9.** | *Suunnittele perustus* joka paikassa: kuori, navigaatio, alapalkki, footer, kaikki sivut ja laskurin oma H1. Tapahtuma `cta_konfiguraattori` + `data-sijainti` ennallaan. Neljäs nimi samalle toiminnolle — aiemmat *Löydä sopiva pilari* (spec 1.0), *Laske pilarien määrä* (16.9.) ja *Mitoita perustus* (speksi, kolme uudempaa sivua). Syy: valikkokohtana *Laske pilarien määrä* olettaa kävijän jo tietävän tarvitsevansa pilareita. Poikkeama spec 1.0:sta, kerrotaan asiakkaalle |
